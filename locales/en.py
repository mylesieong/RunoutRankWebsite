"""English copy for the Runout Rank site — the source language.

Every locales/<code>.py module exposes the same three names:

    LOCALE   the entry from common.LOCALES this module writes
    UI       the chrome strings (nav, footer, byline, store badges, schema prose)
    PAGES    one dict per page: slug, title, description, body, schema, keywords

build.py renders each locale from those three names alone, so a translation is a
copy of this file with the strings replaced — no template or layout changes.
"""
...
UI = dict(
    ...
    store_in_review="In review",
    store_review_aria="In review for the App Store",

    # --- footer ---------------------------------------------------------
    footer_blurb="An absolute pool rating from a ten-table run-out test on your own table. A 0&ndash;100 "
                 "number in one sitting &mdash; no league, no 200-game wait, no account, no internet needed.",
    footer_col_app="The app",
    footer_col_guides="Guides",
    footer_links_app=[
        ("how-it-works.html", "How it works"),
        ("levels.html", "The six levels"),
        ("practice.html", "Practice &amp; training log"),
        ("runout-pro.html", "Runout Pro"),
    ],
    footer_links_guides=[
        ("fargo-rate-alternative.html", "Fargo Rate alternative"),
        ("pool-rating-without-a-league.html", "A rating without a league"),
        ("absolute-vs-relative-pool-rating.html", "Absolute vs relative ratings"),
        ("pool-skill-level-test.html", "Pool skill level test guide"),
        ("faq.html", "FAQ"),
        ("privacy-policy.html", "Privacy policy"),
    ],
    footer_sitemap="Sitemap",
    footer_copyright="&copy; {year} Runout Rank. Written and built by {author}.",
    footer_platforms="Android &amp; iOS &middot; Dark mode only, like the app",
    footer_disclaimer="Fargo Rate and FargoRate are trademarks of their respective owner. "
                      ...
                      "behaviour of those systems and are offered for the reader&rsquo;s own judgement.",

    # --- social / meta --------------------------------------------------
    og_image_alt="Runout Rank — how good are you at pool, really?",

    # --- privacy policy page --------------------------------------------
    privacy_title="Privacy policy | Runout Rank",
    privacy_description="Runout Rank stores your tests, ratings and practice history on your device only. "
                        "This policy explains the analytics the app does use, who data is shared with, and your rights.",
    privacy_h1="Privacy policy",
    privacy_breadcrumb="Privacy policy",
    privacy_lead="Your tests and statistics stay on your device. This page explains everything the "
                 "app does collect, why, and what control you have over it.",

    # --- 404 ------------------------------------------------------------
    not_found_title="Page not found | Runout Rank",
    not_found_description="That page does not exist. Head back to the Runout Rank home page.",

    # --- SoftwareApplication schema -------------------------------------
    app_description="Runout Rank is an absolute billiards skill-rating app. Take a ten-table run-out test on a "
                    "real pool table, get a 0-100 rating and a tier from Rookie to Master in a single session, "
                    "then practise at the level that beats you. The rating measures you against fixed layouts "
                    "rather than against local opponents, so it needs no league, no 200-game history and no "
                    "opponents at all. All data stays on your device.",
    app_features=[
    ...
    app_offer="Free to download. Optional Runout Pro subscription unlocks progress history and CSV export.",
)
...
GUIDE = f"""  <section class="page-head">
    <div class="container">
{breadcrumb(UI, "Pool skill level test")}
...
FARGO_ALT = f"""  <section class="page-head">
    <div class="container">
{breadcrumb(UI, "Fargo Rate alternative")}
...
NO_LEAGUE = f"""  <section class="page-head">
    <div class="container">
{breadcrumb(UI, "A rating without a league")}
...
ABSOLUTE = f"""  <section class="page-head">
    <div class="container">
{breadcrumb(UI, "Absolute vs relative ratings")}
...
PAGES = [
    ...
    dict(slug="how-it-works.html",
         ...
         body=HOW,
         schema=[breadcrumb_schema(LOCALE, UI, "How it works", "how-it-works.html")]),
    ...
    dict(slug="levels.html",
         ...
         description="Rookie, Regular, League, Competitor, Advanced, Master. What changes at each "
                     ...
                     "nothing is locked.",
         body=LEVELS,
         schema=[breadcrumb_schema(LOCALE, UI, "Levels", "levels.html")]),
    ...
    dict(slug="practice.html",
         ...
         description="Endless randomly generated practice layouts at the level you choose, one-tap "
                     ...
                     "table you have run.",
         body=PRACTICE,
         schema=[breadcrumb_schema(LOCALE, UI, "Practice", "practice.html")]),
    ...
    dict(slug="fargo-rate-alternative.html",
         ...
         schema=[article_schema(LOCALE, UI, 
             ...
             breadcrumb_schema(LOCALE, UI, "Fargo Rate alternative", "fargo-rate-alternative.html")],
         published=UPDATED,
    ...
    dict(slug="pool-rating-without-a-league.html",
         ...
         schema=[article_schema(LOCALE, UI, 
             ...
             breadcrumb_schema(LOCALE, UI, "A rating without a league", "pool-rating-without-a-league.html")],
         published=UPDATED,
    ...
    dict(slug="absolute-vs-relative-pool-rating.html",
         ...
         schema=[article_schema(LOCALE, UI, 
             ...
             breadcrumb_schema(LOCALE, UI, "Absolute vs relative ratings", "absolute-vs-relative-pool-rating.html")],
         published=UPDATED,
    ...
    dict(slug="pool-skill-level-test.html",
         ...
         schema=[article_schema(LOCALE, UI, 
             ...
             breadcrumb_schema(LOCALE, UI, "Pool skill level test", "pool-skill-level-test.html")],
    ...
    dict(slug="faq.html",
         ...
         description="Do you need a real table? A league? How is the rating calculated, how does it "
                     ...
                     "questions.",
         body=FAQ,
         schema=[FAQ_SCHEMA, breadcrumb_schema(LOCALE, UI, "FAQ", "faq.html")]),
]
